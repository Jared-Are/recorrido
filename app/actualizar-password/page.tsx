  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (!session) {
        supabase.auth.getSession().then(({ data }) => {
           if (!data.session) {
             // Solo redirigimos si definitivamente no hay sesión después de esperar
             // toast({ title: "Enlace expirado", description: "Por favor solicita uno nuevo.", variant: "destructive" });
             // router.push("/login"); // Descomenta si quieres redirección forzosa
             setVerificando(false); // Dejamos de cargar para mostrar el estado vacío
           }
        });
      }
    }, 3000);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center border-b bg-white/50">
          ...
          <CardDescription>
            Hola <span className="font-bold text-blue-700">{session.user?.user_metadata?.nombre || "Usuario"}</span>. 
